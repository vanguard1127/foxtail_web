...
const ProfilesContainer: React.FC<IProfilesContainerProps> = memo(
  ({
    ...
    const setMatchDlgVisible = (matchDlgVisible, profile, chatID = null) => {
      ...
      if (profile) {
        setState({
          ...
          matchDlgVisible,
          chatID,
          linkedProfiles: [...state.likedProfiles, profile.id]
        });
      } else {
    ...
    const setMsgModalVisible = (msgModalVisible, profile = null) => {
      ErrorHandler.setBreadcrumb("Message Modal visible:" + msgModalVisible);
    ...
    const setMessaged = (profileID) => {
      ...
      setState({
        ...state,
        msgdProfiles: [state.msgdProfiles, profileID],
        msgModalVisible: false
      });
    };
    ...
    if (error) {
      return (
        <ErrorHandler.report error={error} calledName={"searchProfiles"} />
      );
    }
    ...
    if (
      ...
      if (data.searchProfiles.message === "invisible") {
        return <InvisibleProfile t={t} />;
      }
      return <InviteFriends t={t} toggleShareModal={toggleShareModal} />;
    }
